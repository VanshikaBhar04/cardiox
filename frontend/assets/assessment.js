...
function renderAssessmentHistory(items) {
  ...
  // Optional: click to load values back into the form
...
async function loadAssessmentHistory(patient_uid) {
  ...
  // Optional: auto-load the most recent assessment into the form
  // If you want this behaviour, uncomment:
  // if (items.length > 0) loadAssessmentIntoForm(items[0]);
}
...
(async function init() {
  ...
  // 🔹 NEW: If editing, load the specific assessment into the form
